...
def video_completion(args):
    ...
    for filename in sorted(filename_list):
        img_h_orig, img_w_orig = np.array(Image.open(filename)).shape[:2]
        img_h = math.floor(img_h_orig / 8) * 8
        img_w = math.floor(img_w_orig / 8) * 8
        video_orig = np.array(Image.open(filename).resize((img_w, img_h))).astype(np.uint8)
        video.append(torch.from_numpy(video_orig).permute(2, 0, 1).float())
    ...
    if args.mode == 'video_extrapolation':
        ...
        for filename in sorted(filename_list):
            ...
            mask_img = np.array(Image.open(filename).resize((img_w, img_h)).convert('L'))
            mask.append(mask_img)

            # Dilate 15 pixel so that all known pixel is trustworthy
            flow_mask_img = scipy.ndimage.binary_dilation(mask_img, iterations=25)
